...
public class IPEventListenerProvider implements EventListenerProvider {
    ...
    private void checkUserLocation(Event event){
        QueryResult queryResult = influxDB.query(new Query("select \"ipAddress\", \"userId\", \"location\" from \"14d\".event where \"userId\" = '" + event.getUserId() + "'ORDER BY \"time\" DESC limit 3", nameInfluxDB)); // interogarea pentru achizitionarea locatiei
        JSONObject myJson = new JSONObject(queryResult);
        JSONArray responseLenght = myJson.getJSONArray("results").getJSONObject(0).getJSONArray("series").getJSONObject(0).getJSONArray("values"); // parcurgerea ierarhiei seriilor pentru achizitia ulterioara a datelor
        try{
  		  for(int i = 0; i < responseLenght.length(); i++){
  			  String json_data = myJson.getJSONArray("results").
			  		  ...
			  		  getJSONArray("values").
			  		  getJSONArray(i).
			  		  getString(3); // achizitia locatiei din ierarhie
	  		  locationList.add(json_data); // adaugarea locatiei in lista de locatii
        ...
        if(responseLenght.length() == 1){
            log.info("only one authentication"); // verificare daca este prima autentificare
        }
        else if(locationList.get(0).toString().equalsIgnoreCase(locationList.get(1).toString()) == false) { //verificare daca locatiile coincid
        	sendEmail(event); // apelare metoda de trimitere e-mail
        }
    }
    ...
    private String ipToLocation(String ip) throws IOException{
        ...
        try {
            URL ipapi = new URL("https://ipapi.co/" + ip + "/city"); // construire URL
            URLConnection c = ipapi.openConnection(); // deschidere conexiune
            c.setRequestProperty("Content-Type", "java-ipapi-client"); // trimitere request
            BufferedReader reader = new BufferedReader(
                    new InputStreamReader(c.getInputStream())
            );
            String line;
            while ((line = reader.readLine()) != null) {
                log.info(line); // citirea rezultatului
            }
            reader.close();
        } catch(IOException e){
    ...
    private void sendEmail(Event event) {
    	...
    	String contentForEmail = "Unrecognized IP location\n\n" +
                ...
                "Client: " + event.getClientId(); // construire antet e-mail
