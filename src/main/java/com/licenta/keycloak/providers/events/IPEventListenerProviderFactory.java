...
public class IPEventListenerProviderFactory implements EventListenerProviderFactory {
    ...
    private final String retentionPolicyInfluxDB = getConnectionDetails("INFLUXDB_DB_RETENTION_POLICY", "14d"); // politica de retentie este ca datele sa fie retinute timp  de 14 zile
    ...
    public void init(Config.Scope config) {
        ...
        try {
            influxDB = InfluxDBFactory.connect(influxUrl, influxdbUser, influxdbPassword); // conexiunea
            influxDB.createRetentionPolicy(retentionPolicyInfluxDB, nameInfluxdbDB, retentionPolicyInfluxDB, null, 1); // crearea politicii de retentie
        } catch (Exception ex) {
    ...
    private String getConnectionDetails(String envVariableName, String defaultEnvValue) {
        String result = System.getenv(envVariableName); // se preia numele variabilei
        return result == null ? defaultEnvValue : result; // se atribuie valoarea sau default
    }
}
